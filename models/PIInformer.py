...
#         self.upper_limit = 5
#         self.lower_limit = 0
#         # self.conv = ConvLayer(configs.d_model)
#         self.linear = nn.Linear(configs.d_model*2,4)
...
#
#     def forward(self, dec_out, cost_time):
#         # dec_out = self.conv(dec_out)
#         dec_out_max = F.max_pool1d(dec_out.permute(0, 2, 1), dec_out.shape[1]).squeeze(-1)
#         dec_out_avg = F.avg_pool1d(dec_out.permute(0, 2, 1), dec_out.shape[1]).squeeze(-1)
#         dec_out = torch.cat([dec_out_max,dec_out_avg],dim=-1)
...
class PhysicalModule(nn.Module):
    def __init__(self, configs):
        ...
        self.lower_limit = 0
        # self.conv = ConvLayer(configs.d_model)
        self.linear_Qd = nn.Linear(configs.d_model * (configs.seq_len+configs.pred_len), 2)
        self.linear_Ed = nn.Linear(configs.d_model* (configs.seq_len+configs.pred_len), 2)
        # self.sigmoid = nn.Sigmoid()
        self.std = configs.std
        self.mean = configs.mean
        self.nominal_capacity = configs.nominal_capacity
    ...
    def forward(self, dec_out, cost_time):
        # dec_out = self.conv(dec_out)
        # dec_out = F.max_pool1d(dec_out.permute(0, 2, 1), dec_out.shape[1]).squeeze(-1)
        out_Qd = self.linear_Qd(dec_out.reshape(dec_out.shape[0],-1))  # [B, 2]
        out_Ed = self.linear_Ed(dec_out.reshape(dec_out.shape[0],-1))  # [B, 2]

        scale_factors = torch.ones_like(out_Qd[:, :]).unsqueeze(1)  # [B,1, 2]

        As = torch.cat([out_Qd[:, :1], out_Ed[:, :1]], dim=-1).unsqueeze(1)  # [B, 1, 2]
        Bs = torch.cat([out_Qd[:, 1:], out_Ed[:, 1:]], dim=-1).unsqueeze(1)  # [B, 1, 2]
        lower_limit = torch.ones_like(As) * self.lower_limit
        # upper_limit = torch.ones_like(As) * self.upper_limit
        Bs = torch.where(Bs < lower_limit, lower_limit, Bs)
        # Bs = torch.where(Bs > upper_limit, upper_limit, Bs)
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, cost_time,
        ...
        # tmp_cost_time = cost_time.unsqueeze(-1)
        # x_enc = torch.cat([x_enc, tmp_cost_time[:, :self.seq_len, :]], dim=-1)
        # x_dec = torch.cat([x_dec, tmp_cost_time], dim=-1)
        ...
        dec_out = self.decoder(dec_out, enc_out, x_mask=dec_self_mask,
                               cross_mask=dec_enc_mask)  # [B, L_(label_len+pred_len), Dm]

        # Physics
        PI_out, As, BS = self.PIModule(dec_out, cost_time)
        out = PI_out
